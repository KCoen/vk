...
// Package khr_extended_flags provides bindings for the VK_KHR_extended_flags extension.
// Author: KHR
...
// Description:
//
// This extension adds a new VkFormatFeatureFlagBits4KHR format feature
// flag type, supporting up to 64 additional flags beyond those defined by
// VkFormatFeatureFlagBits2; and a new VkFormatProperties4KHR
// structure to extend VkFormatProperties2.
// The new structure does not replace format feature flag bits specified by
// existing commands and structures, unlike VkFormatProperties3.
// Instead, it adds new flags to the existing APIs.
// Initially, no new format flag bits are defined.
//
// This extension also adds a new VkImageCreateFlagBits2KHR image create
// flag type, supporting up to 32 additional flags, and a new
// VkImageCreateFlags2CreateInfoKHR structure to extend
// VkImageCreateInfo and other commands that specify image create flag
// bits.
...
// All the existing VkImageCreateFlagBits are given corresponding
// VkImageCreateFlagBits2KHR bits with the same meaning.
// Initially, no new image create flag bits are defined.
//
// This extension also adds a new VkImageUsageFlagBits2KHR image usage
// flag type, supporting up to 32 additional flags, and a new
// VkImageUsageFlags2CreateInfoKHR structure to extend
// VkImageCreateInfo and other commands that specify image usage flag
// bits.
// The new structure duplicates image usage flag bits specified by existing
// commands and structures, while also adding up to 32 new flags.
// All the existing VkImageUsageFlagBits are given corresponding
// VkImageUsageFlagBits2KHR bits with the same meaning.
// Initially, no new image usage flag bits are defined.
//
// The extension is infrastructure used because the API is running out of
// available bits in the VkFormatFeatureFlagBits2 and
// VkImageUsageFlagBits types, and will soon thereafter run out of bits
// in VkImageCreateFlagBits.
//
// Finally, this extension adds all the same 64-bit flag types first introduced
// by VKKHRmaintenance5.
