// Code generated by vk-gen. DO NOT EDIT.

// Package ext_attachment_feedback_loop_layout provides bindings for the VK_EXT_attachment_feedback_loop_layout extension.
// Author: EXT
// Contact: Joshua Ashton @Joshua-Ashton
//
// Other Extension Metadata:
//
// Last Modified Date:
//
//	2022-04-04
//
// IP Status:
//
//	No known IP claims.
//
// Contributors:
//   - Joshua Ashton, Valve
//   - Faith Ekstrand, Collabora
//   - Bas Nieuwenhuizen, Google
//   - Samuel Iglesias Gonsálvez, Igalia
//   - Ralph Potter, Samsung
//   - Jan-Harald Fredriksen, Arm
//   - Ricardo Garcia, Igalia
//
// Description:
//
// This extension adds a new image layout,
// VKIMAGELAYOUTATTACHMENTFEEDBACKLOOPOPTIMAL_EXT, which allows
// applications to have an image layout in which they are able to both render
// to and sample/fetch from the same subresource of an image in a given render
// pass.
//
// Version History:
//
//   - Revision 2, 2022-04-04 (Joshua Ashton)
//
//   - Renamed from VALVE to EXT.
//
//   - Revision 1, 2021-03-09 (Joshua Ashton)
//
//   - Initial draft.
package ext_attachment_feedback_loop_layout
