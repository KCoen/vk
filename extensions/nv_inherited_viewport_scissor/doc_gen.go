...
// Package nv_inherited_viewport_scissor provides bindings for the VK_NV_inherited_viewport_scissor extension.
// Author: NV
...
// DISCUSSION:
//
// We considered both adding a new fvkCmdSetViewportDepthNV function, and
...
// The main issue with adding a new fvkCmdSetViewportDepthNV function is
...
//
// The proposal to modify vkCmdSetViewport would avoid these issues.
...
// state of VkCommandBuffer that must be re-initialized.
//
// (2) Why are viewport depth values specified as a partial VkViewport
// struct, rather than a leaner depth-only struct?
// DISCUSSION:
//
// We considered adding a new sVkViewportDepthNV struct containing only
// minDepth and maxDepth.
// However, as application developers would need to maintain both a
// VKNVinheritedviewportscissor code path and a fallback code path (at
...
// separate sVkViewportDepthNV and VkViewport arrays for each code
// path.
//
// Version History:
//
