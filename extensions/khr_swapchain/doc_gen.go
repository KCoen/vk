...
// Package khr_swapchain provides bindings for the VK_KHR_swapchain extension.
// Author: KHR
...
// Description:
//
// The VKKHRswapchain extension is the device-level companion to the
// VKKHRsurface extension.
...
// vkQueuePresentKHR call will be applied atomically as one operation.
...
// RESOLVED:
//
//   - VKSUBOPTIMALKHR - Presentation will still succeed, subject to
...
// received from vkAcquireNextImageKHR.
//
...
//
// 12) Can VKPRESENTMODEMAILBOXKHR provide non-blocking guarantees
...
// images when the call to vkAcquireNextImageKHR was made.
...
// that has generated a VKSUBOPTIMALKHR return code while still using
// the old swapchain?
//
// RESOLVED:
...
// VkSurfaceCapabilitiesKHR::pname:maxImageCount to indicate there are no
...
// VkSurfaceCapabilitiesKHR::pname:currentExtent to indicate the size of
...
// VkSurfaceCapabilitiesKHR::pname:maxImageExtent to indicate there is no
...
// transform bit: VKSURFACETRANSFORMROTATE90BITKHR.
...
//
// 18) Can surfaces ever not support VKMIRRORNONE? Can they support
// vertical and horizontal mirroring simultaneously? Relatedly, should
// VKMIRRORNONE[_BIT] be zero, or bit one, and should applications be
...
// VKMIRRORNONEBIT | VKMIRRORHORIZONTALBIT, or a transform
...
// VKMIRRORHORIZONTALANDVERTICAL_BIT to represent a simultaneous
...
// color space is VKCOLORSPACESRGBNONLINEAR_KHR.
//
...
// driver, a value of VKCOMPOSITEALPHAINHERITBIT_KHR is provided like
// for surface transforms.
//
// 27) Is vkCreateSwapchainKHR the right function to return
// VKERRORNATIVEWINDOWINUSEKHR, or should the various
...
// VkSurfaceKHR objects that may be created for the same native window if
// such a requirement makes sense on a particular platform, but a global
// requirement is only sensible at the swapchain level.
//
// Examples:
//
// The example code for the VKKHRsurface and VKKHRswapchain
...
//     vkwsipersistentswapchainimages extension.
//
...
//
//   - Added issue 9, and the related VKSUBOPTIMALKHR result code.
//
//   - Renamed VKOUTOFDATEKHR to VKERROROUTOFDATE_KHR.
//
...
//
//   - Moved some structures to VKEXTKHRswapchain to resolve the
...
//     VKIMAGELAYOUTPRESENTSOURCE_KHR.
//
...
//     VKDEFINENONDISP_HANDLE macro, and no longer extending VkObjectType
...
//     VKSTRUCTURETYPEQUEUEPRESENTINFOKHR to be consistent with the
...
//
//   - Noted that support for VKPRESENTMODEFIFOKHR is required.
...
//     James Jones)
//
//   - Moved the surface transform enums to VKWSIswapchain so they could be
//     reused by VKWSIdisplay.
//
...
//
//   - Renamed from VKEXTKHRdeviceswapchain to VKEXTKHR_swapchain.
//
...
//
//   - Added error result VKERRORSURFACELOSTKHR.
//
...
//
//   - Added VKPRESENTMODEFIFORELAXED_KHR.
//
//   - Revision 63, 2015-10-15 (Daniel Rakos)
//
//   - Moved surface capability queries to VKEXTKHR_surface.
//
//   - Revision 64, 2015-10-26 (Ian Elliott)
//
//   - Renamed from VKEXTKHRswapchain to VKKHR_swapchain.
//
